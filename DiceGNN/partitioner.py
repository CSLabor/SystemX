...
def initial_guess(n_total, t_cpu, t_dma, t_model, t_gpu, t_uva):
    # initial guess, a monotone min max problem of three lines
    y1 = lambda x : x * t_cpu
    y2 = lambda x : x * t_dma + (n_total - x) * t_uva
    y3 = lambda x : n_total * t_model + (n_total - x) * t_gpu
    x1 = n_total * t_gpu / (t_cpu + t_gpu - t_dma)     # cpu time = pcie time
    x2 = n_total * (t_gpu + t_model) / (t_gpu + t_cpu) # cpu time = gpu time
